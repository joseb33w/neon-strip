...
func build_area(rec: Dictionary, scene_parent: Node, player: Node3D, world_main: Node,
	...
	var nav := _build_room(root, size, _col(rec.get("ground", [0.3, 0.33, 0.38])))
...
func _build_room(root: Node, size: float, ground_col: Color) -> NavigationRegion3D:
	...
	root.add_child(nav)
	_box(nav, Vector3(0, -0.5, 0), Vector3(size * 2, 1, size * 2), ground_col)
